...
@Component({
  selector: 'app-listservicetransport',
  templateUrl: './listservicetransport.component.html',
  styleUrls: ['./listservicetransport.component.scss']
})
export class ListservicetransportComponent implements OnInit {
     ...
     // console.log(res)
      for(const item of res.data){
        this.User.forEach((res:any)=>{
          if(item.transporteurid==res.id.toString()){
        ...
        if(this.userconnected.data.role == "admin"){
          ...
          if(item.transporteurid.toString() == this.userconnected.data.id.toString()){
            this.Users.push(item)
          }
        }
  ...
  onSearchChange(searchValue: any): void {
    ...
    for(const item of this.InitialeVar){
      if(item.lieudepart.toLowerCase().includes(searchValue.target.value.toLowerCase())|| item.destination.toLowerCase().includes(searchValue.target.value.toLowerCase()) ){
        this.Users.push(item)
        this.changeDetector.detectChanges();
      }else{
      //  this.Users.push(item)
